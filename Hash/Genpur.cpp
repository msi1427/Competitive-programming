...
int main()
{
    Fast
    F[0] = 1;
    for(ll i = 1; i < MAX; i++) F[i] = F[i - 1] * hs;
    cin>>n;
    FLR(i,n) {
        string s;
        cin>>s;
    ...
    string ans = vs[0];
    H[0] = 0;
    repl(i, 1, sz(ans)) H[i] = H[i - 1] * hs + ans[i-1];
    repl(i,1,n-1){
        string ss = vs[i];
        G[0]=0;
        repl(j, 1, sz(ss)) G[j] = G[j - 1] * hs + ss[j-1];
        char ch = ss[0];
        ll pos = 0;
        for(ll j=max(0LL,(ll)sz(ans)-sz(ss));j<sz(ans);j++){
            if(ans[j]==ch){
                ll xx = sz(ans)-j;
                if(xx<=sz(ss)){
                    ull yy = seg1(j+1,sz(ans));
                    ull zz = seg2(1,xx);
                    if(yy==zz){
                        pos = xx;
                        break;
