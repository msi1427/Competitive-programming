...
int main()
{
    ...
    vector<pll>vv;
    repl(i,1,n)  if(debt_to_get[i]<0) vv.eb(-debt_to_get[i],i);
    vector<tll>ans;
    repl(i,1,n){
        while(debt_to_get[i]>0){
            ll cur = min(debt_to_get[i],vv.back().ff);
            ans.eb(vv.back().ss,i,cur);
            debt_to_get[i]-=cur;
            vv.back().ff-=cur;
            if(vv.back().ff==0) vv.pop_back();
        }
    }
    cout << sz(ans) << "\n";
    for(auto& p:ans) cout << get<0>(p) << " " <<  get<1>(p) << " " << get<2>(p) << "\n";
}
/*
